...
execution_log = "Server is ready."

@app.route('/')
def home():
    return f"Nabd24 Reel Server is Alive! 🚀<br><br><b>Status Log:</b><br><pre>{execution_log}</pre>"

def generate_voice_sync(text, output_path):
...
def scrape_news_article(url):
    try:
        ...
        # استخراج النص المختصر للخبر
        paragraphs = soup.find_all('p')
        full_text_list = [p.text.strip() for p in paragraphs if len(p.text.strip()) > 20]
        content = " ".join(full_text_list[:2]) # فقرتين لتسريع الصوت والفيديو
        
        if len(content) < 20:
            content = title
            
        return title[:100], content[:300], img_url
    except Exception as e:
        return "تطورات إخبارية عاجلة", "تابع حصريات منصة نبض 24 الإخبارية.", None
...
def background_processing(news_url, page_token):
    ...
    try:
        execution_log = f"1. Scraping URL: {news_url}"
        title, description, img_url = scrape_news_article(news_url)
        full_speech = f"عاجل. {title}. {description}"

        execution_log = f"2. Generating Voice..."
        generate_voice_sync(full_speech, audio_path)

        audio_info = MP3(audio_path)
        duration = int(audio_info.info.length) + 1

        execution_log = f"3. Processing Image..."
        ...
        if img_url:
            try:
                ...
                # فتح وتعديل خفيف وآمن للذاكرة
                im = Image.open(image_path).convert('RGB')
                im = im.resize((720, 1280), Image.Resampling.BILINEAR)
                im.save(image_path, quality=85)
                image_downloaded = True
            except Exception as img_err:
                execution_log += f"\nImage warning: {img_err}"
        ...
        execution_log = f"4. Running FFmpeg..."
        ...
        if result.returncode != 0:
            execution_log = f"❌ FFmpeg Error: {result.stderr.decode('utf-8', errors='ignore')[-200:]}"
            return

        execution_log = f"5. Uploading to Facebook..."
        ...
        with open(video_path, "rb") as video_file:
            ...
            execution_log = f"🎉 Success! FB Response: {fb_response}"

    except Exception as e:
        execution_log = f"❌ Critical Error: {str(e)}\n{traceback.format_exc()[-200:]}"
